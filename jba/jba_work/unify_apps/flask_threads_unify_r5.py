...
def check_tweets_list(potential_full: list) -> bool:
    """
    Checks if the list of tweets has tweets
    if does not have returns False else returns True
    """
...
def set_api(twitter_tokens):
    '''    
     Creates a Twitter API object
     Verifies credentials if ok returns the API object else exits with sys.exit(1)
     signaling an abnormal exit
    '''
    ...
    # section names are delimited with [] characters (i.e [twitter]). The pairs are separated either with : or =.
...
# ---------------------------------------------------------------------------------------------------------
# assign twitter_user.properties configuration file to twitter_tokens variable
...
def sentiment_scores(sentance: str) -> dict:
    '''
    Return Polarity_Scores for each tweet
    '''
...
def tweets_list_output(tweets_list):
    """
    Return full text for each tweet
    """
    start = time.time()
    output = []
    for tweet in tweets_list:
        ...
        # logger.debug(f"full_text: '{translated}'")
...
def sentiment_eval_pUDF_wrapper(pdf_series):
    """
     Evaluate tweet's sentiment in respect the compound score of each tweet
    - positive -> 1,
    - negative -> -1,
    - neutral -> 0
    """
...
def scrapetweets(t_user, t_search_words, t_date_since, t_date_until, t_lang):
    ...
    for tweet in tweets_list:
        ...
        user_desc = tweet._json['user']['description']
        user_location = tweet._json['user']['location']
        user_following = tweet._json['user']['friends_count']
        user_followers = tweet._json['user']['followers_count']
        user_totaltweets = tweet._json['user']['statuses_count']
        user_created_at = tweet._json['user']['created_at']
        ...
        line = {'query_user': t_user,
                ...
                'user_totaltweets':user_totaltweets,
    ...
    # +----------+-------------------+------------------------------+------------------------------+----+---------------+-------------+-------------------+-------------------------+------------------+-----------+------------------------------+--------------+--------------+--------------+----------------+------------------------------+
    # |query_user|             id_str|                  search_words|                          text|lang|favourite_count|retweet_count|         created_at|                 hashtags|       user_id_str|screen_name|                     user_desc| user_location|user_following|user_followers|user_totaltweets|               user_created_at|
    # +----------+-------------------+------------------------------+------------------------------+----+---------------+-------------+-------------------+-------------------------+------------------+-----------+------------------------------+--------------+--------------+--------------+----------------+------------------------------+
    # |       joe|1477779862022729728|[coffeeIsland OR (coffee is...|We visited the largest #cof...|  en|              0|            1|2022-01-02 23:12:29|                 [coffee]|776103870791618560| vectorborg|                              |Ekerö, Sverige|            72|          1337|          100837|Wed Sep 14 17:02:36 +0000 2016|
    # |       joe|1477776114097418240|[coffeeIsland OR (coffee is...|We visited the largest #cof...|  en|              2|            1|2022-01-02 22:57:36|[coffee, GreenEnergy, ag]|910281043961188352|STEAMFresno|Champion for Design Science...|    Fresno, CA|          1066|           364|             991|Tue Sep 19 23:14:47 +0000 2017|
    # +----------+-------------------+------------------------------+------------------------------+----+---------------+-------------+-------------------+-------------------------+------------------+-----------+------------------------------+--------------+--------------+--------------+----------------+------------------------------+
    ...
    # clean_text_sdf.show(2, truncate=30)
...
def detachedProcessReadSentimentResultsFromMongDB(spark, request_data):
    ...
    pipeline = str({'$match': {'$and': [{'created_at': {'$gte': {'$date': date_since_str}}}, {
                   'created_at': {'$lte': {'$date': date_until_str}}}], 'search_words': search_words, 'query_user':query_user, 'lang':lang}})
    # print("\npipeline : ", pipeline, "\n")
    ...
    # ----------------------------
    # ----------------------------
    # When using filters with DataFrames or the Python API, the underlying Mongo Connector code constructs an aggregation pipeline 
    # to filter the data in MongoDB before sending it to Spark.

    # mongodf = spark.read.format("mongo").option("database", "tweets_DB").option(
    #     "collection", "tweets_sentiment_scores").load()

   
    # mongodf = mongodf.filter((mongodf['created_at'] <= date_until_str) & (mongodf['created_at'] >= date_since_str)).filter(mongodf['search_words'] == search_words).filter(mongodf['query_user'] == query_user).filter(mongodf['lang'] == lang)
    # mongodf = mongodf.filter((mongodf['created_at'] <= date_until_str) & (mongodf['created_at'] >= date_since_str)).filter(mongodf['query_user'] == query_user).filter(mongodf['search_words'] == search_words).filter(mongodf['lang'] == lang)
    # mongodf = mongodf.filter(((mongodf['created_at'] <= date_until_str) & (mongodf['created_at'] >= date_since_str)))
    ...
    # To manage thread-local data, just create an instance ol local(or a subclass)
...
def detachedProcessSearchTweets(spark, tweets_data):
    ...
    # +----------+-------------------+------------------------------+------------------------------+----+---------------+-------------+-------------------+-------------------------+------------------+-----------+------------------------------+--------------+--------------+--------------+----------------+------------------------------+------------------------------+
    # |query_user|             id_str|                  search_words|                          text|lang|favourite_count|retweet_count|         created_at|                 hashtags|       user_id_str|screen_name|                     user_desc| user_location|user_following|user_followers|user_totaltweets|               user_created_at|                    clean_text|
    # +----------+-------------------+------------------------------+------------------------------+----+---------------+-------------+-------------------+-------------------------+------------------+-----------+------------------------------+--------------+--------------+--------------+----------------+------------------------------+------------------------------+
    # |       joe|1477779862022729728|[coffeeIsland OR (coffee is...|We visited the largest #cof...|  en|              0|            1|2022-01-02 23:12:29|                 [coffee]|776103870791618560| vectorborg|                              |Ekerö, Sverige|            72|          1337|          100837|Wed Sep 14 17:02:36 +0000 2016|We visited the largest #cof...|
    # |       joe|1477776114097418240|[coffeeIsland OR (coffee is...|We visited the largest #cof...|  en|              2|            1|2022-01-02 22:57:36|[coffee, GreenEnergy, ag]|910281043961188352|STEAMFresno|Champion for Design Science...|    Fresno, CA|          1066|           364|             991|Tue Sep 19 23:14:47 +0000 2017|We visited the largest #cof...|
    # +----------+-------------------+------------------------------+------------------------------+----+---------------+-------------+-------------------+-------------------------+------------------+-----------+------------------------------+--------------+--------------+--------------+----------------+------------------------------+------------------------------+
    ...
    #                     StructField(user_following,LongType,true),
    #                     StructField(user_followers,LongType,true),
    #                     StructField(user_totaltweets,LongType,true),
