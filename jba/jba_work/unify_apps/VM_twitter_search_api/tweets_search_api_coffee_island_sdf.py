...
spark=SparkSession.builder.appName("Spark_and_Pandas_twitter_dfs").master("local[*]") .getOrCreate()
...
tweets_list = tweepy.Cursor(api.search, q=['#coffeeisland* OR (COFFEE ISLAND)'],since= str(since), until=str(until),tweet_mode='extended', lang='en').items(50)
...
sdf = spark.createDataFrame(Row(**x) for x in output)
...
df_reg = spark.createDataFrame([('100-200',)], ['str'])
df_reg.show()

df_reg = df_reg.select('str', regexp_extract('str', r'(\d+)-(\d+)', 1).alias('d'))
df_reg.show()
# print(df_reg['d'].collect())
# [Row(d='100')]
print(df_reg.select('d').collect())

df_reg = spark.createDataFrame([('foo',)], ['str'])
df_reg.show()
df_reg = df_reg.select('str', regexp_extract('str', r'(\d+)', 1).alias('d'))
df_reg.show()
print(df_reg.select('d').collect())
# [Row(d='')]

df_reg = spark.createDataFrame([('aaaac',)], ['str'])
...
# %%
# sdf.write.format('csv').mode('overwrite').save('output_cof_island_sdf_esxtended.csv', escape = '"')

sdf.write.format('parquet').mode('overwrite').save('output_cof_island_sdf_parquet', escape = '"')
...
# %%
from pyspark.sql.types import StructType, StructField, IntegerType, DateType, StringType, TimestampType, ArrayType
...
schema = StructType([
    ...
    StructField("uder_id_str", StringType(), True),
    StructField("screen_name", StringType(), True)
    ])

# Exception has occurred: AnalysisException
...
sdf_2 = spark.read.parquet('output_cof_island_sdf_parquet',schema=schema, header=False,escape = '"', multiLine=True) #, escape = '"',
# %%
# sdf.show(2, truncate =30)
