...
def scraptweets(search_words, date_since, numTweets, numRuns):
    ...
    for i in range(0, numRuns):
        ...
        start_run = time.time()
        tweets = tweepy.Cursor(api.search, q=search_words, since=date_since, tweet_mode='extended', lang='en').pages(numRuns)
        for tweet in tweets:
            logger.info(f"tweet_id_str: {'-'*30}")
            # logger.info(f"created_at: {tweet.created_at_datetime}")
