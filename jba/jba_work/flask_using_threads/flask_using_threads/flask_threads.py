...
def detachedProcessFunction(spark, test_data):
    ...
    for row in range(rows):
        dict_output = {'_id':mongoAggregationdf.select("_id").collect()[row][0]['oid'],
        ...
        dictionary_copy = dict_output.copy()
        # WRONG !!!!!
...
def tweets_sentiment_results():
    ...
    if request.method == 'POST':
        global p1; global t1; global i; global q1
        ...
        t1 = threading.Thread(target=detachedProcessFunction,
                                args=(spark,test_data), name=f'thread{i}')
        t1.start()
        print(f'\n\n\ntweets_sentiment_results - ThreadName = {t1.name}\n\n\n')

        #p1.join()
        #t1.join()
        # Consumer thread 
        t1.join()
        ret = jsonify({'result':q1.get()})
        print("**********************************************")
        print(ret)
