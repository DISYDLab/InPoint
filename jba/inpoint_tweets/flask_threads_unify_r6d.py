...
from app_package.errors import InvalidParameter, NullPointerException
...
# =========================================================================================================
# ---------------------------------------------------------------------------------------------------------
database = "tweets_DB_new"
collection = "tweets_sentiment_scores"
...
def start_spark():
    ...
    spark = SparkSession \
        ...
        .config(f"spark.mongodb.input.uri",
                "mongodb://0.0.0.0:27017/{database}.{collection}?readPreference=primaryPreferred") \
        .config(f"spark.mongodb.output.uri", "mongodb://0.0.0.0:27017/{database}.{collection}") \
        .getOrCreate()
...
def detachedProcessReadSentimentResultsFromMongDB(spark, request_data):
    ...
    mongodf = spark.read.format("mongo").option("database", database).option(
    ...
    # +--------------------------+------------------------------+-------------+-------------------+---------------+--------+-------------------+----+-------------+------------+-------------+----------+-------------+-----------+------------------------------+--------------+------------------------------+------------------------------+-------------------+
    # Check if there is a tweets_DB_new DB
    ...
    if df_columns:
        ...
        # if mongoDB tweets_DB_new is not empty
        if dates_length:
            ...
            status = 400
            message = f"No results found - No data in MongoDB {database}.{collection}  - for date_since: {date_since} to date_until: {date_until}"
...
def detachedProcessSearchTweets(spark, tweets_data): 
    ...
    # Default: true
    mongo_tweetsDF.write.format("mongo").option("database", database).option(
    ...
    # ]
    # )
    # mongo_tweetsDF = spark.read.format("mongo").option("database", database).option(
    #     "collection", collection).option('schema', schema).load()
    # mongo_tweetsDF.select('*').show(10, truncate=20)
    # print('\n', '='*60)
    # print(f'mongo_tweetsDF Schema: ')
    # print(mongo_tweetsDF.printSchema())

    # mongo_tweetsDF = spark.read.schema(schema).format("mongo").option("database", database).option(
    #     "collection", collection).load()
...
def basic_error(e):
    ...
    # logging.exception() method, logs a message with level ERROR and adds exception information to the message. 
    # logging.exception("Exception occurred")
 
    if len(e.args) == 1 and e.status:
        # jsonify: Serialize data to JSON and wrap it in a ~flask.Response with the application/json mimetype.
        ...
        return {"code": 400,  "name":"ValidationError", "description": f"{message1}  + {str(e.args)}"}, 400
    
    elif len(e.args) == 2 and e.args[1].find('%Y-%m-%d'):
        # print(*e.args)
        return {"code": 400, "name":"ValidationError", "description": f"{str(e.args)}"}, 400
...
def query_twitter_and_do_sentiment_analysis():
    ...
    if output_list_not_empty:
            ...
            if mongodf.count() != 0:
                ...
                if diff_dates:
                    ...
                    if is_date_until_greater_than_date:
                        message = f"Not Accepted - some dates for search_query already in MongoDB {database}.{collection}  - you can only enter the following inclusive dates={[date.isoformat() for date in diff_dates]}, but keep in mind these should be contiguous dates and date_until is not an inclusive date"
                        raise InvalidParameter(message, status)
                    else:
                    ...
                    message = f"Not Accepted - same search_query already in MongoDB {database}.{collection} - for the date_since={date_since} and date_until={date_until} you provided"
                    raise InvalidParameter(message, status)
        ...
        return {"message": "Accepted", status:202}, 202
    else:
        status = 400
