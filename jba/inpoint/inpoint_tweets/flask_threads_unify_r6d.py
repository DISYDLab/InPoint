...
def detachedProcessReadSentimentResultsFromMongDB(spark, request_data):
    ...
    mongodf = spark.read.format("mongo").option("database", "tweets_DB_new").option(
    ...
    if df_columns:
        ...
        if dates_length:
            ...
            status = 204
            message = f"Not Accepted - empty query no data in in MongoDB tweets_DB_new.tweets_sentiment_scores  - for date_since: {date_since} to date_until: {date_until}"
...
def detachedProcessSearchTweets(spark, tweets_data):
    ...
    # Default: true
    mongo_tweetsDF.write.format("mongo").option("database", "tweets_DB_new").option(
    ...
    # ]
    # )
    # mongo_tweetsDF = spark.read.format("mongo").option("database", "tweets_DB_new").option(
    ...
    # mongo_tweetsDF = spark.read.schema(schema).format("mongo").option("database", "tweets_DB_new").option(
    #     "collection", "tweets_sentiment_scores").load()
...
def query_twitter_and_do_sentiment_analysis():
    ...
    if output_list_not_empty:
        ...
        if df_columns:
            ...
            if mongodf.count() != 0:
                ...
                if diff_dates:
                    ...
                    message = f"Not Accepted - same search_query already in MongoDB tweets_DB_new.tweets_sentiment_scores - for the date_since={date_since} and date_until={date_until} you provided"
                    raise InvalidParameter(message, status)
        ...
        raise InvalidParameter(message, status)
        # return {"message": "Not Accepted - empty result_list - change your search_query"}, 400
