...
def buildGneo4j(G, user):
    conn = Neo4jConnection(uri="bolt://localhost:7687", user="neo4j", pwd="1234")
    print("Connection established-listening to port: 7687")
    #qexist = "MATCH (u:User) RETURN u";
    graph_query = 'MATCH (p:'+user+') OPTIONAL MATCH (p)<-[:Followed_by]-(c) RETURN {parent : p.myid, child : {name :collect( c.myid)}}'

    namegraph_neo4j = 'CALL gds.graph.create(\'test02\', \'User4\', \'Followed_by\')'
    namedgraph = conn.query(namegraph_neo4j);

    q2 = 'CALL gds.louvain.stream(\'test02\') YIELD nodeId, communityId RETURN gds.util.asNode(nodeId).myid AS name, communityId ORDER BY name ASC'
    mycommunities = conn.query(q2);
    print(mycommunities)
    for n in mycommunities:
        n = list(n);
        qupdate = 'MATCH (p:User4{myid:\''+n[0]+'\'}) SET p.communityId=\''+str(n[1])+'\' RETURN p';
    ...
    print(graph_query)
    neo4jgraph = conn.query(graph_query);
    for n in neo4jgraph:
        n = list(n);
        parent = n[0]['parent'];
        print('parent = ', parent)
        for c in n[0]['child']['name']:
            print('child = ', c)
            G.add_edge(int(parent),int(c),weight=1.0)
